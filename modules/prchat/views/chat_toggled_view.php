        ...
        if (obj.hasClass('on')) {
          $(message).each(function(key, value) {
            ...
            if (value.reciever_id == userSessionId) {
              ...
              $('.pusherChatBox#' + id + ' .logMsg .msgTxt').prepend('<div class="conversation_me"><img class="myProfilePic" data-toggle="tooltip" data-placement="top" title=" <?= _l('chat_sent_at') ?>' + value.time_sent_formatted + '" src="' + fetchUserAvatar(userSessionId, value.user_image) + '"/></br><div class="message_container"><p data-toggle="tooltip"  data-placement="top" id="' + value.id + '" data-title="' + (!isViewed ? '<?= _l('chat_not_seen'); ?>' : '<?= _l('chat_msg_seen'); ?> ' + moment(value.viewed_at).format('h:mm:ss A, MMM YYYY')) + '" class="you">' + value.message + '</p>' + optionsMore + '</div>' + not_seen_icon + '<div class="fixinline"></div></div>');
            }
          });
  ...
  user_messages_events_small.bind('message_seen', function(messages) {
    ...
    if (Array.isArray(messages)) {
      for (var i = 0; i < messages.length; i++) {
        ...
        var seen_at = l_seen + moment("<?= date('Y-m-d H:i:s'); ?>").format('h:mm:ss A, MMM YYYY');
